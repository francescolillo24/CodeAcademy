	...
	public class EFDidactisService : IDidactisService
	{
		...
		public IEnumerable<CourseEdition> Search(EditionSearchInfo info)
		{
			if (info.Start != null || info.End != null)
			{
				...
				{
					throw new BusinessLogicException("I criteri di ricerca non possono comprende  allo stesso tempo date e richiesta su futuro e passato");
				}
			}
			...
			if (info.Start != null && info.End != null)
			{
				...
				{
					throw new BusinessLogicException("La data di inizio non può essere successiva a quella di fine");
				}
			}

			if (info.InTheFuture == true && info.InThePast == true)
			{
				throw new BusinessLogicException("Non è possibile richiedere edizioni sia nel passatro che nel futuro");
			}
