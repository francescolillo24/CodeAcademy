...
namespace AcademyEFPersistance.Services
{
	...
	public class EFPeopleService : IPeopleService
	{
		...
		{
			var existingEnrollment = studentRepo.FindEnrollmentByStudentIdAndEditionId(data.IdStudent, data.IdEdition);
			if(existingEnrollment != null)
            {
				throw new BusinessLogicException($"Lo studente con id {data.IdStudent} gia iscritto al edizione {data.IdEdition}");
            }
